...
<c:forEach items="${astorList }" var="user" varStatus="status">
	<tr>
		<td><input name="" type="checkbox" value="" /></td>
		<td>${user.getYb_userid() }</td>
		<td>${user.getYb_realname() }</td>
		
		<td>${user.getYb_username() } </td>
		
		<td>${user.getYb_sex() }</td>
		
		<td>${user.getYb_identity() }</td>
		<td>${user.getYb_schoolname() }</td>
		<td>${user.getYb_userhead() }</td>
		
		<td><a href="<%=path%>/AdminServlet?method=queryById&otId=${user.getId() } " class="tablelink">查看</a>
		 	<a href="<%=path%>/AdminServlet?method=delete&otId=${user.getId() } " class="tablelink">修改</a>
		 </td>
	</tr>
</c:forEach>		
			</tbody>
