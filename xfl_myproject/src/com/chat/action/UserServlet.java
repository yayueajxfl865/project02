...
import com.chat.utils.BaseServlet;
...
public class UserServlet extends BaseServlet {
	...
	public String sendMessage(HttpServletRequest req,HttpServletResponse resp) throws IOException{
		...
		String flagInforid=req.getParameter("flagInfor");
		
		Region.savePeopleChatId=flagInforid;
	...
	public String kick(HttpServletRequest req,HttpServletResponse resp) throws IOException{
		...
		session.invalidate();
		// 重定向到页面
		resp.sendRedirect(req.getContextPath()+"/main_chat.jsp");
		return null;
	}
	
	/**
	 * 易班账户登录的功能，根据易班id号唯一识别User对象
	 */
	public String login(HttpServletRequest req,HttpServletResponse resp){
		...
		try {
			...
			String yb_userid=(String) req.getSession().getAttribute("yb_userid");
			User existUser = us.getUserByybId(yb_userid);//这里返回的user是已经成功登录的user;
			if (existUser == null) {
				// 用户登录失败
				req.setAttribute("msg", "易班用户登录失败!");
				return "/shop.jsp";
			} else {
				// 用户登录成功
				...
				sourceMessage += "系统公告：<font color='gray'>"
						+ existUser.getYb_username() + "走进了聊天室！</font><br>";
				application.setAttribute("message", sourceMessage);

				//req.getRequestDispatcher("/main_chat.jsp").forward(req, resp);
				resp.sendRedirect(req.getContextPath() + "/main_chat.jsp");
				return null;
			}
		} catch (Exception e) {
